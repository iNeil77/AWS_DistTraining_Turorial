...
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y libhwloc-dev libnuma-dev libaio-dev
...
RUN pip3 install accelerate \
    ...
    &&  DS_BUILD_OPS=1 \
        ...
        DS_BUILD_EVOFORMER_ATTN=0 pip install --no-build-isolation deepspeed==${DEEPSPEED_VERSION} \
...
COPY ./Code_Preference_Learning/Training/Axolotl_Framework /workspace/axolotl
